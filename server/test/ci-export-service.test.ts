...
describe('ExportService.exportInstallation — open_pr call order + persistence timing', () => {
  it('calls commitFiles, then findOpenPr, then openPullRequest, and only calls upsertPublished after they succeed', async () => {
    ...
    expect(result.secret_value).toBe('sk-or-test-key');
  });
...
describe('ExportService.bulkUpdate', () => {
  it('one installation failing to commitFiles still returns a full 2-entry results array with the other ok:true, and never calls openPullRequest', async () => {
    ...
    let openPrCalls = 0;
    github.openPullRequest = async (...args: Parameters<typeof github.openPullRequest>) => {
      openPrCalls += 1;
      throw new Error('bulkUpdate must never call openPullRequest');
    };
