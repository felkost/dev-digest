...
/**
 * `POST /export-ci`'s response extends the persisted `CiExport` contract with
 * a one-time secret value the client shows for copying (`ExportService`'s
 * `CiExportResult` — see export-service.ts's own doc comment on why that
 * field is never added to the shared `CiExport` contract itself).
 */
const CiExportResultSchema = CiExport.extend({ secret_value: z.string() });
  ...
  app.post(
    '/repos/:repoId/agents/:agentId/export-ci',
    {
      schema: {
        params: RepoAgentParams,
        body: CiExportInput,
        response: { 200: CiExportResultSchema },
      },
      config: {
        rateLimit: {
          max: 2,
