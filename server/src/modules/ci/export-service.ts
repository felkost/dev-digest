...
/**
 * `exportInstallation`'s response extends the persisted/reusable `CiExport`
 * contract with a one-time secret value the client displays for copying.
 * Deliberately NOT added to the shared `CiExport` contract itself — that type
 * is also used for contexts (e.g. a future list/detail read) where re-exposing
 * a secret would be a leak risk. This stays a local, route-facing shape.
 */
export type CiExportResult = CiExport & { secret_value: string };
...
export class ExportService {
    ...
    if (existing && existing.disconnectedAt === null && existing.agentId !== agentId) {
      throw new AppError(
        ...
        'This repository already has DevDigest installed for a different agent. ' +
          'Disconnect it first, or choose another repository.',
        409,
      );
    }
  ...
  async exportInstallation(
    ...
    const secretValue = await this.container.secrets.get('OPENROUTER_API_KEY');
    if (!secretValue) {
      ...
      active.map((inst) =>
        ...
        queue.add(async (): Promise<CiBulkUpdateOutcome> => {
          try {
            ...
            // Never openPullRequest here (AC-40).
            ...
            return { installation_id: inst.id, ok: true, pr_url: null, error: null };
          } catch (err) {
            // Isolated per-installation — one failure never aborts the rest
            // (AC-41), and a failed installation's row is left untouched.
            return { installation_id: inst.id, ok: false, pr_url: null, error: String(err) };
          }
        }, { throwOnTimeout: true }),
      ),
    );
