...
**Status:** implemented <!-- approved 2026-07-11; amended 2026-07-11 post architecture-review; implemented 2026-07-12 — see §13 -->
**Date:** 2026-07-11
...
- **AC-5** (Ubiquitous): The system SHALL compute per-block and per-run token counts using one single, centrally shared counting function — not two independently maintained implementations — consumed identically by the studio and by CI, so reported counts from the two environments are directly comparable.
...
- **AC-19** (Ubiquitous): The system SHALL apply one single, centrally shared boilerplate-classification rule — not two independently maintained pattern lists — before prompt assembly, whether a review executes inside CI or in the studio.
...
- **AC-23** (Ubiquitous): The system SHALL use one single, centrally defined token-count threshold value in the studio and in CI — not two independently maintained constants.
- **AC-24** (Ubiquitous): The system SHALL make the same single, shared token-counting function used in the studio directly available to CI review execution — not a separately implemented CI-only counter — so the identical token-budget chunking decision can be applied there as in the studio.
...
- WHERE AC-2's map-reduce chunk-overhead counting and WS5's token-budget chunking decision (AC-21–AC-25) require a token count during the review engine's own mode-selection and chunk-assembly step, the system SHALL implement that counting as a pure, injected-dependency function with zero DB access, zero file I/O, and zero direct environment-variable reads — verify: the engine's existing hermetic test suite (stubbed LLM provider, no network) continues to pass unmodified for this addition.
- WHERE AC-1's per-prompt-block token counting runs as a server-side (and CI-side) post-hoc measurement over the review engine's already-returned prompt assembly — an addition at the caller, not a change to the engine's internal pipeline, mirroring the existing per-context-document token counting already computed this way — the system SHALL implement it as a synchronous, zero-I/O computation reusing the same shared counting function as AC-2/WS5's in-engine case — verify: no new engine-internal call site is introduced for this per-block breakdown.
- WHERE cache-metric capture and boilerplate classification run inside the review engine's shared package, the system SHALL implement them as pure, injected-dependency functions with zero DB access, zero file I/O, and zero direct environment-variable reads — verify: the engine's existing hermetic test suite (stubbed LLM provider, no network) continues to pass unmodified for these additions.
...
| File boilerplate / core / wiring classification | [deterministic: repo-intel] — existing L03 mechanical classification, relocated to the shared review-engine package (reviewer-core) as the single canonical implementation consumed by both the studio and CI, zero LLM calls |
| Per-prompt-block token counts | [deterministic: repo-intel] — mechanical counting via a single counting function, exported from the shared review-engine package (reviewer-core) alongside its existing single-sourced provider implementation, consumed identically by the studio and by CI, zero LLM calls |
...
|---|---|---|---|
| block_token_counts | list of (block name, token count) pairs | Per-slot token counts for the assembled prompt (system, skills, specs, callers, repo_map, pr_description, diff/user), computed by the single shared counting function used identically in the studio and CI | A block missing from the list means that slot was not part of this run's prompt |
...
| map_reduce_threshold_tokens | integer | The single, centrally shared token threshold (same value and same counting method in the studio and in CI) used to decide/chunk this run's map-reduce split | N/A — always present when map-reduce mode is used |
...
**Amended 2026-07-11 (architecture review).** An architecture review of this approved spec flagged that WS1/WS4/WS5's studio↔CI parity requirements (AC-5, AC-19, AC-23, AC-24) named the right observable outcome ("same method, not two independently maintained implementations") but did not mandate where that single-sourced implementation must live to make the parity real — leaving room for a plan that grows a second, independently maintained copy in CI and keeps the two "in sync by convention." The user approved closing this gap with the amendments below; AC-5, AC-19, AC-23, and AC-24 were tightened accordingly (see §5), and §7/§8/§9 were updated to match. No AC's intent, no workstream, and no goal/non-goal changed — these are precision/placement clarifications, not new requirements.

- **Canonical token-counter home (WS1, WS5).** The concrete token-counting implementation SHALL live in the shared review-engine package (reviewer-core), exported alongside its existing single-sourced LLM-provider implementation — the codebase's own precedent for a capability both the studio and CI need living in exactly one place. It SHALL be injected as a pure function into the engine's own mode-selection and chunk-assembly step for AC-2/WS5's in-engine counting, and the studio's server-side container and CI both consume that same exported function for AC-1's post-hoc per-block counting (see §7's split). The review engine remains side-effect-free throughout: the counting function takes text and returns a count, with no environment read and no concrete tokenizer choice baked into the engine's own pipeline logic — the concrete implementation is supplied to the engine as an injected dependency, never assumed by it.
- **Canonical boilerplate-classifier home (WS4).** The boilerplate/core/wiring file classifier SHALL likewise live in the shared review-engine package (reviewer-core), consumed identically by the studio and by CI, rather than as a studio-only rule that CI would otherwise have to duplicate or go without. The classification logic is pure path/pattern matching with zero I/O, so this relocation changes only where the rule is defined, not what it does — AC-15 through AC-20's behavior, including the Dockerfile/workflow/`.env.example` exception in AC-18, is unaffected.
- **Review-intent override ownership (WS3).** Wiring AC-13's dead review-intent override SHALL NOT be done by adding a new, second import from the reviews capability into the settings capability — that cross-capability import already exists once, for the risk-brief narrative's own feature-model resolution, and the module-boundary convention (a module imports only from its own files, the shared-contracts layer, and the platform layer) does not sanction a second one. Instead, feature-model override resolution for both the review-intent override (AC-13) and the risk-brief narrative's existing override SHALL be relocated to the platform layer — the same layer that already owns cheap/capable model-tier routing, and the module-boundary convention's sanctioned route for a capability multiple modules legitimately need — and consumed by the reviews capability from there. This resolution SHALL be exercised from a single place, not duplicated at each of intent classification's current call sites, so AC-13's override behavior stays consistent everywhere intent classification runs. This changes which layer produces the behavior, not AC-11/AC-12/AC-13's observable behavior itself.
- **Cache-control marking is adapter-only (WS2, minor).** For AC-8, the map-reduce system prefix is already byte-identical across a run's chunks, so the provider-facing adapter can decide to mark it cacheable directly from the message content it is already sending — no new "is this a map-reduce run" signal needs to be added to the engine's request contract for AC-8 to hold. This keeps AC-9's "additive metadata only" guarantee trivially satisfied — no new data needs to enter the prompt-building path to make the marking decision.

**Implemented 2026-07-12.** All 12 plan steps (3 waves) landed and verified via `/implement` (plan: [docs/plans/2026-07-11-cost-surgery-backend-plan.md](../plans/2026-07-11-cost-surgery-backend-plan.md)); committed as `aed7464`. Migration `0026` applied (26 rows backfilled `case_kind='review_finding'`). Final plan-verifier tally: 55 ✅ / 2 ⚠️ (both user-accepted) / 0 ❌, zero un-executed rows. Gates green: server typecheck + hermetic suite, reviewer-core 74/74, agent-runner 26/26 (the 2 pre-existing `conventions-extractor` Windows-path failures are unrelated to this spec), and the Docker-gated `settings-models.it.test.ts` 4/4 integration proof (AC-11/12/13). Three in-flight fixes were applied during the run and are recorded in the SDD journal: (1) `resolveIntentModel` returns `{provider,model}` so a cross-provider `review_intent` override drives both client and model; (2) `generateRiskBriefNarrative` relocated `reviews/`→`platform/risk-brief.ts`, closing the eval→reviews cross-import; (3) `excludeBoilerplateFiles` rewritten to an exact per-`diff --git b/<path>` section filter. The CI runner ships current source automatically: `NccRunnerBundler` rebuilds the `agent-runner` `ncc` bundle fresh on every export (never cached/checked in), so no manual re-bundling is required for a target repo to pick up this spec's cost/boilerplate instrumentation.
