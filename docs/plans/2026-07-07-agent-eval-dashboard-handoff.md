...
2. **Скільки «останніх»** — фіксований ліміт (рекоменд. 20), enforced серверно, без пагінації в v1.
...
7. **Compare — повний**: word-level diff системного промпту + робочий **Promote** (мутація активного
   ...
   graceful-degrade (AC-22). Механізм персистенції (стовпець vs JSON) — рішення планувальника.
