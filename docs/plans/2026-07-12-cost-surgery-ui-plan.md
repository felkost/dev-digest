...
3. **Per-block token display (AC-25 tail), descoped 2026-07-12 (user).** Spec A's `cost_report` per-block/cache/boilerplate fields are mirrored into the client contracts (Step 1) but no surface renders them — the mockups don't show a per-block breakdown, and AC-25 is a conditional non-fabrication guard, satisfied for the one Spec A field actually displayed (cost-by-model, null→"—"). A future follow-up may add a per-block/cache/boilerplate section to `RunTraceDrawer`'s `TraceBody` (rendering `block_token_counts`, `cached_input_tokens`, `cache_control_applied`, `excluded_boilerplate_files/_tokens`, `map_reduce_*`, with unavailable→"—" for older runs); out of scope here.
