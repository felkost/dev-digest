...
- **2026-07-04 [Decision]** — Blast-radius summary is stored denormalized in `pull_requests.blast_summary`, written once when the review completes, not recomputed per request. The symbol-index walk cost 300ms+ on large PRs and the summary is immutable after the run finishes. `server/src/modules/blast/service.ts:88`
