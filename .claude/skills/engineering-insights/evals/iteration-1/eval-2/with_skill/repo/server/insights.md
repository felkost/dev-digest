...
## Mistakes
- **2026-06-15 [Mistake]** — `db:seed` is not idempotent for `review_runs`; running it twice creates duplicate run records that break the agent detail page. Check the `review_runs` count before re-seeding. `server/src/db/seed.ts:44` (2026-07-04 — also duplicates `agent_runs` rows, not only `review_runs`; verify both counts before re-seeding.)
