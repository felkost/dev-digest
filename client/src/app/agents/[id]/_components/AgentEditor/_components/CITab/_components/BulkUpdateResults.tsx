    ...
    <div style={s.bulkResults} role="status" aria-live="polite">
      ...
      {result.results.map((r) => (
        <div key={r.installation_id} style={s.bulkResultRow}>
          ...
          {r.ok ? (
            // Link straight to the PR that carries the update — the config only
            // lands once that PR is merged, so surfacing it here is the whole
            // point of the update (a bare "OK" hid where the change actually went).
            r.pr_url ? (
              <a href={r.pr_url} target="_blank" rel="noopener noreferrer" style={{ textDecoration: "none" }}>
